...
public final class Danbooru implements ApiCommand {
    @Nonnull
    private static final System.Logger LOGGER = System.getLogger(Danbooru.class.getName());
    ...
    public static void invoke(@Nonnull SlashCommandInteractionEvent event) {
        LOGGER.log(System.Logger.Level.INFO, "Danbooru command invoked by {0} ({1}) of guild {2} ({3})", 
        ...
        if (danbooruClient.getApiKey() == null) {
            LOGGER.log(System.Logger.Level.WARNING, "Failed to invoke Danbooru command due to missing API token.");
