...
public final class Weather implements ApiCommand {
    @Nonnull
    private static final System.Logger LOGGER = System.getLogger(Weather.class.getName());
    ...
    public static void invoke(@Nonnull SlashCommandInteractionEvent event) {
        LOGGER.log(System.Logger.Level.INFO, "Weather command invoked by {0} ({1}) of guild {2} ({3})", 
        ...
        try {
            LOGGER.log(System.Logger.Level.INFO, "Attempting to retrieve data for location: {0}", location);
            ...
            LOGGER.log(System.Logger.Level.WARNING, "Failed to execute weather command due to missing weather token.");
            event.reply("Weather command unavailable currently.").queue();
        } catch (IOException e) {
            LOGGER.log(System.Logger.Level.WARNING, "Failed to retrieve weather data for location: {0}.", location);
            event.reply(String.format("Error retrieving weather data for location: %s.", location)).queue();
        } catch (InterruptedException e) {
            LOGGER.log(System.Logger.Level.WARNING, "Interrupted while retrieving posts for location: {0}.", location);
