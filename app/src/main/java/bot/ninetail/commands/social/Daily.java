...
public final class Daily implements SocialCommand {
    ...
    public static void invoke(@Nonnull SlashCommandInteractionEvent event) {
        Logger.log(LogLevel.INFO, String.format("Daily command invoked by %s (%s) of guild %s (%s)", 
        ...
        if (CoinsRegistry.getInstance() == null || CoinsRegistry.getInstance().getData() == null) {
            event.reply("Sorry, database features are currently unavailable.").setEphemeral(true).queue();
            return;
        }

        try (Connection conn = CoinsRegistry.getInstance().getData().getConnection()) {
            ...
            if (result.getBoolean("claimed")) 
                ...
                try {
                    ...
                    if (timeResult.next()) {
                        ...
                        event.reply(String.format("⏳ You already claimed today. Next claim available in %d hours and %d minutes.", 
                                                 hoursLeft, minutesLeft)).setEphemeral(true).queue();
                    } else {
            ...
            Logger.log(LogLevel.ERROR, String.format("Database error in Daily command: %s", e.getMessage()));
