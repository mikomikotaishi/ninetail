...
public final class AudioIdentifier {
    ...
    /**
     * Rewrites links that the YouTube source cannot recognise into their equivalent on the main
     * YouTube domain.
     *
     * YouTube Kids serves ordinary YouTube videos under a domain of its own, which the source
     * does not match against, so those links would otherwise resolve to nothing at all.
     *
     * @param url The URL supplied by the user.
     * @return The equivalent URL on a recognised domain, or the original URL if no rewrite applies.
     */
    @Nonnull
    private static String normaliseUrl(@Nonnull String url) {
